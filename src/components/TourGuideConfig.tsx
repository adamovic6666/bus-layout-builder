...
interface TourGuideConfigProps {
  ...
  onDriverSeatsChange: (seats: string[]) => void;
  totalSeats: number;
}

export const TourGuideConfig = ({ tourGuideSeats, driverSeats, onTourGuideSeatsChange, onDriverSeatsChange, totalSeats }: TourGuideConfigProps) => {
  ...
  const removeDriverSeat = (seatId: string) => {
    const newSeats = driverSeats.filter(seat => seat !== seatId);
    onDriverSeatsChange(newSeats);
    toast(`Removed Seat ${getSeatDisplayLabel(seatId)} from driver seats`);
  };

  const clearAllDriverSeats = () => {
    onDriverSeatsChange([]);
    toast("All driver seats cleared");
  };
  ...
  return (
    <>
      ...
      <Card className="shadow-lg border-border/60">
        ...
        <CardContent className="space-y-4">
          {/* Current Driver Seats */}
          {driverSeats.length > 0 && (
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-muted-foreground">Current:</span>
                <Button 
              ...
              <div className="flex flex-wrap gap-1">
                {driverSeats.map((seatId) => (
                  <Badge key={seatId} variant="secondary" className="flex items-center gap-1">
                    Seat {getSeatDisplayLabel(seatId)}
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => removeDriverSeat(seatId)}
                      className="h-4 w-4 p-0 hover:bg-destructive hover:text-destructive-foreground"
                    >
                      <X className="h-2 w-2" />
                    </Button>
          ...
          {driverSeats.length === 0 && (
            <div className="text-center text-muted-foreground py-2">
              <p className="text-xs">No driver seats selected</p>
              <p className="text-xs">Ctrl+click on bus seats to mark as driver</p>
            </div>
          )}
        </CardContent>
      </Card>
